from src.model.PINN import MLP
from src.model.PINN_original import PINN
...
net = MLP(input_dim=2, output_dim=1, hidden_layers=[25]*4, activation='GELU')
net = net.to(device)
mse_cost_function = torch.nn.MSELoss() # Mean squared error
optimizer = torch.optim.Adam(net.parameters(), lr=0.001)
PINN_dwo = PINN(net, data, pde, bcic, bc_n, device)
